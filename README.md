...
- **Reported density is a sampling artifact.** Each agent is binned into one 1.5 m cell and multiplied by `tickets_sold / n_agents`, so one agent registers 16.7 p/m² at a time and every hotspot density is an integer multiple of that quantum. The number therefore tracks the agent-count slider rather than the crowd — the same schedule peaks at 66.7 p/m² with 2,000 agents and 37.5 with 8,000, and 46% of the venue reads as crush risk. No constant can fix this; it needs a measurement radius (Gaussian kernel), which would also retire the 0.56 factor. Treat hotspot *locations* as directional and the densities as meaningless.
- **The optimizer can only swap filled slots**, never move an act into an empty one — 21 of the fixture's 55 grid slots are unreachable.
